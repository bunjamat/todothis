...
const ProgrammerTodo = () => {
  ...
  const TaskCard = ({ task }) => (
    <Alert
      key={task.id}
      className="relative overflow-hidden group bg-white hover:bg-gray-50 transition-all duration-300"
    >
            ...
            <div className="flex flex-wrap gap-2 mt-2">
              ...
              {task.tags &&
                task.tags.map((tag, index) => (
                  <span
                    key={index}
                    className="text-xs flex items-center gap-1 px-3 py-1 rounded-full bg-gray-100 text-gray-700 hover:bg-gray-200 transition-colors duration-300"
                  >
        ...
        <CardContent>
          ...
          <Tabs defaultValue="pending" className="w-full">
            ...
            <TabsContent value="pending" className="mt-6 space-y-4">
              {filterTasks(tasks.filter((t) => t.status === "pending")).map(
                (task) => (
            ...
            <TabsContent value="in-progress" className="mt-6 space-y-4">
              {filterTasks(tasks.filter((t) => t.status === "in-progress")).map(
                (task) => (
                  <TaskCard key={task.id} task={task} />
                )
              )}
            ...
            <TabsContent value="completed" className="mt-6 space-y-4">
              {filterTasks(tasks.filter((t) => t.status === "completed")).map(
                (task) => (
                  <TaskCard key={task.id} task={task} />
                )
              )}
